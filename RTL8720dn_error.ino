...
extern "C" {
  ...
  // MCC (Multi-Channel Concurrent) tarama: AP modunda 5GHz dahil TÜM kanalları tek tek tarar.
  // Standart wifi_scan_networks() AP modunda sadece mevcut 2.4GHz kanalında çalışır.
  ...
  // wext_private_command lib_arduino.a içinde tanımlı — wext_set_txpower wrapper'ı için kullanılır.
  int  wext_private_command(const char *ifname, char *cmd, int show_msg);
  // wifi_set_country wifi_conf.h içinde zaten tanımlı — burada yeniden bildirmiyoruz.
}
...
#define SCAN_RESULT_WAIT_MS         35000   // MCC tarama 38 kanal × ~150ms ≈ 6s; güvenli üst limit
...
rtw_result_t raw_scan_handler(rtw_scan_handler_result_t *malloced_scan_result) {
  ...
  if (malloced_scan_result->scan_complete != RTW_TRUE) {
    ...
    if (net.ssid.length() > 0) {
      ...
      // Duplicate kontrol — aynı SSID ama farklı bant (2.4GHz/5GHz) AYRI tutulur
      bool dup = false;
      for (auto &existing : scan_temp) {
        bool same_band = (existing.channel >= 36) == (net.channel >= 36);
        if (existing.ssid == net.ssid && same_band) {
          dup = true;
...
void scanNetworkTask(void *param) {
  ...
  // MCC tarama: AP modunda 5GHz dahil 38 kanalı (2.4GHz 1-13 + 5GHz 36-165) sırayla tarar.
  // wifi_scan_networks() AP modunda yalnızca mevcut kanaldaki ağları görür — 5GHz kaçırılır.
  wifi_scan_networks_mcc(raw_scan_handler, NULL);
...
void sendStartPage(WiFiClient &client) {
  ...
  if (networks.size() > 0) {
    for (int i = 0; i < (int)networks.size(); i++) {
      ...
      bool is5g = (networks[i].channel >= 36);

      client.print("<li class='net-item' onclick=\"document.getElementById('r"); client.print(i); client.print("').checked=true\">");
      client.print("<input type='radio' name='net_idx' id='r"); client.print(i); client.print("' value='"); client.print(i); client.print("' required>");
      ...
      if (is5g) client.print(" <span style='font-size:.7rem;background:#2980b9;color:#fff;padding:2px 5px;border-radius:4px;'>5GHz</span>");
      else      client.print(" <span style='font-size:.7rem;background:#27ae60;color:#fff;padding:2px 5px;border-radius:4px;'>2.4GHz</span>");
      client.print("</div>");
      client.print("<div class='net-meta'>Sinyal: "); client.print(rssiBar(networks[i].rssi));
      client.print(" | Kanal: "); client.print(networks[i].channel);
      client.print("</div></div></li>");
    }
...
void handleClient(WiFiClient &client) {
  ...
  if (!ap_switched) {
    ...
    if (request.startsWith("POST") && path == "/start_ap") {
      String sel_idx_str = parsePostParam(body, "net_idx");
      if (sel_idx_str.length() == 0) { sendStartPage(client); return; }

      int sel_idx = sel_idx_str.toInt();
      target_channel = 6;
      ...
      if (sel_idx >= 0 && sel_idx < (int)networks.size()) {
        NetworkInfo &net = networks[sel_idx];
        strncpy(target_ssid, net.ssid.c_str(), MAX_SSID_LEN - 1);
        target_ssid[MAX_SSID_LEN - 1] = '\0';
        target_channel = net.channel;
        target_enc     = net.enc;
        target_sec     = net.raw_sec;
        memcpy(target_bssid, net.bssid, 6);
        // 5GHz ağ seçilmişse: bu ağı 5GHz hedef yap, 2.4GHz kardeşi ara
        if (net.channel >= 36) {
          target_5g_channel = net.channel;
          memcpy(target_5g_bssid, net.bssid, 6);
          for (auto &n : networks) {
            if (n.ssid == net.ssid && n.channel < 36) {
              target_channel = n.channel;
              memcpy(target_bssid, n.bssid, 6);
              break;
            }
          }
        }
...
// ── agentSetTxPower — wext_private_command üzerinden KM0 TX gücü ayarı ──────
// wifi_util.c:763'teki wext_set_txpower() lib_arduino.a'ya dahil edilmemiş.
// Doğru format: "txpower patha=X pathb=X" — her iki RF zinciri aynı anda set.
// Geçerli indeks aralığı: 0–63 (Realtek RTL8720DN donanım maksimumu = 63).
// NOT: wifi_start_ap() ve wifi_set_mode() çağrıları güç register'larını
// sıfırlar; bu yüzden her AP başlatmasından SONRA yeniden çağrılmalıdır.
static int agentSetTxPower(const char *ifname, int poweridx) {
  if (poweridx > 63) poweridx = 63;   // donanım sınırını aşma
  char buf[40];
  snprintf(buf, sizeof(buf), "txpower patha=%d pathb=%d", poweridx, poweridx);
  return wext_private_command(ifname, buf, 0);
}

// Her AP başlatmasından sonra tüm donanım performans kısıtlarını kaldır
static void applyPerformanceSettings(void) {
  // ── 1. Bölgesel TX güç kısıtı tamamen kaldır ─────────────────────────
  // RTW_COUNTRY_WORLD1 (0x20): Worldwide, bazı kanal/hız güç tablolarını uygular.
  // RTW_COUNTRY_GLOBAL (0x41): Hiçbir bölgesel kısıt yok — tüm güç tablosu bypass.
  wifi_set_country(RTW_COUNTRY_GLOBAL);

  // ── 2. Her iki RF zinciri için maksimum TX güç indeksi ────────────────
  // patha + pathb aynı anda set (yalnızca patha yazmak pathb'yi etkilemiyor)
  agentSetTxPower("wlan0", 63);
  agentSetTxPower("wlan1", 63);

  // ── 3. Tüm güç tasarrufu modlarını kapat ──────────────────────────────
  // wifi_disable_powersave() IPS+LPS ikisini de kapatır ancak bazı SDK
  // sürümlerinde sadece kısmen etkili. wext_private_command ile kesin kapat.
  wifi_disable_powersave();
  wext_private_command("wlan0", "lps 0", 0);   // Light Power Save kapat
  wext_private_command("wlan0", "ips 0", 0);   // Idle Power Save kapat

  // ── 4. 5GHz DFS adaptivity kilidini kaldır ───────────────────────────
  // adaptivity_en 0: LBT (Listen-Before-Talk) mekanizmasını devre dışı bırakır.
  // Bu mekanizma 5GHz DFS kanallarında (52-64, 100-140) TX gücünü ve süresini
  // kısıtlar. Kapatmak 5GHz iletim kapasitesini maksimuma çıkarır.
...
void setup() {
  ...
  // ── MAX TX POWER — wifi_start_ap SONRASINDA uygulanmalı ─────────────────
  // wifi_start_ap() ve wifi_on() çağrıları güç register'larını kendi değerleriyle
  // ezer. Bu yüzden TX güç ayarı AP başlatmasından SONRA yapılmalıdır.
  // applyPerformanceSettings: GLOBAL + patha=63 pathb=63 + lps/ips=0 + adaptivity_en=0
...
void loop() {
  if (pending_ap_switch) {
    ...
    // AP switch sonrası tüm performans ayarlarını yeniden uygula
    applyPerformanceSettings();
  ...
  if (ap_switched && ap_running_channel != -1 && ap_running_channel != target_channel) {
    ...
    // Kanal değişimi sonrası tüm performans ayarlarını yeniden uygula
    applyPerformanceSettings();
